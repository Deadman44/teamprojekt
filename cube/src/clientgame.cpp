...
void updateworld(int millis)        // main game update loop
{
    ...
    {     
        ...
        demoplaybackstep();
        if(!demoplayback) //normales spiel.....
        {
			/* hier schießt der spieler!
			*/
            if(getclientnum()>=0) shoot(player1, worldpos);     // only shoot when connected to server // daher clientnum > 0
