...
void putint(uchar *&p, int n) 
	/*
	was bedeutet *&p?
	eine referenz to pointer übergabe,
	quasi call-by-reference für einen pointer,
	ist dann hilfreich, wenn der pointer, welche innerhalb der funktion geändert wird
	auch außerhalb so geändert bleiben soll
	ohne das & zeichen wird call-by-value gemacht, d.h. der pointer wird kopiert
	änderungen innerhalb der fkt die am pointer passieren sind außerhalb nicht sichtbar
...
int getint(uchar *&p)
{
	...
	*/
    ...
    int c = *((char *)p);
    p++; //pointer rutscht weiter...., gilt auch fürs processing
...
/*
beschreibt wie groß die einzelnen message-pakete sein sollen
...
{ 
    ...
    SV_EXT, 0, SV_ALRS, 2, SV_DUMMYALRS, 2,//TP
    -1
};
